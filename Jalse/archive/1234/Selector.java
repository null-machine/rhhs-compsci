...
public class Selector {
  ...
  private void renderBox(Graphics g, Color color, int x, int y, int w, int h) {
    int offset = index * 8;
    g.setColor(color);
    g.fillRect(
  ...
  private void renderText(Graphics g, Color color, int x, int y, double xPres, double yPres, String text) {
    int offset = index * 8;
    g.setColor(color);
  ...
  public void render(Graphics g, int index, double cellSize) {
    ...
    switch (state) {
      case AI:
        ...
        renderText(g, Color.WHITE, 3, 5, 0.5, 0.6, "MOVE DOWN"); // set by controller
        renderText(g, Color.WHITE, 3, 6, 0.5, 0.6, " TO JOIN");
        break;
      case MODELS:
