...
public class Engine extends JPanel {
  ...
  private void collide(Racer racer, Wall wall) {
    // shift out and apply force
    GeoVector dir = racer.pos.sub(wall.pos);

    racer.collide();

    racer.pos = racer.pos.add(dir.mult(0.1));
    racer.vel.x = -racer.vel.x;
    racer.vel.y = -racer.vel.y;
  }
    ...
    public void run() { // throws InterruptedException
      while (running) {
        for (int i = 0; i < racers.size(); ++i) {
          ...
          for (Wall wall : walls) {
            if (!racer.collided && racer.collider.intersects(wall.collider)) {
              collide(racer, wall);
            }
          }
          ...
          if (!racer.collided && finish.collider.contains(racer.collider)) {
          // if (!racer.collided && finish.collider.intersects(racer.collider)) {
            racer.win();
            racers.remove(racer);
          }
