...
/**
* A moving wall that's controlled by players and bots.
...
public class Racer extends Wall implements Updatable {
  ...
  * @param tag Whether this racer is inside a wall.
  */
  ...
  */
  ...
  protected void updateNode() {
    double currDist = readPos.dist(pn.pos);
    double nextDist = (pn.next != null) ? readPos.dist(pn.next.pos) : Double.POSITIVE_INFINITY;
    double prevDist = (pn.prev != null) ? readPos.dist(pn.prev.pos) : Double.POSITIVE_INFINITY;

    while (nextDist < currDist) {
      pn = pn.next;
      currDist = readPos.dist(pn.pos);
      nextDist = (pn.next != null) ? readPos.dist(pn.next.pos) : Double.POSITIVE_INFINITY;
    }
    while (prevDist < currDist) {
      pn = pn.prev;
      currDist = readPos.dist(pn.pos);
      prevDist = (pn.prev != null) ? readPos.dist(pn.prev.pos) : Double.POSITIVE_INFINITY;
    }
  }
  ...
  */
  public void collide() {
    ...
    vel = vel.neg();
    GeoVector dir = pn.pos.sub(readPos).normalize();
