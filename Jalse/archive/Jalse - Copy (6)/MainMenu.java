...
public class MainMenu extends JPanel implements Updatable {
  ...
  private void genMap() {
    ...
    WallMap gm = new WallMap(abstractMap, options.getWallWidth()); // TODO

    gameState.walls = gm.getWalls();
    gameState.finish = gm.getFinish();

    for (Racer racer : gameState.racers) {
      racer.pn = abstractMap.getStart();
    }
  }
